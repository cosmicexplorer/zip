...
///
/// If a file larger than this threshold attempts to be written, [`ZipWriter`] will raise an
    ...
    pub fn find_and_parse<T: Read + io::Seek>(
        ...
        while pos >= search_upper_bound {
            ...
            if reader.read_u32::<LittleEndian>()? == CENTRAL_DIRECTORY_END_SIGNATURE {
                ...
                let cde_end_pos = reader.seek(io::SeekFrom::Start(pos))?;
                return CentralDirectoryEnd::parse(reader).map(|cde| (cde, cde_end_pos));
            }
